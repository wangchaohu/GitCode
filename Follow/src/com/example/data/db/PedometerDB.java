...
public class PedometerDB {
	...
	public DateTimeData queryNow(DateTimeData data) {
		...
		if (cursor.moveToFirst()) {
				...
				datas=new DateTimeData(currentTime,time+data.time-5,step+data.step,
						calorimetry+data.calorimetry,kilometer+data.kilometer);
		}
		return datas;
	}
	...
	public void updateUser(User user) {
		if (user != null) {
			...
			values.put("groupId", user.getGroupId());
			values.put("today_step", user.getToday_step());
			db.update("user", values, "objectId= ? ", new String[] { user.getObjectId() });
		}
	}
	...
	public void updateStep(Step step) {
		if (step != null) {
			...
			values.put("date", step.getDate());
			values.put("userId", step.getUserId());
			db.update("step", values, "userId = ? and date = ?", new String[] { step.getUserId(), step.getDate() });
		}
	}
	...
	public void updateGroup(Group group) {
		if (group != null) {
			...
			db.update("group", values, "Id = ?", new String[] { String.valueOf(group.getID()) });
		}
	}

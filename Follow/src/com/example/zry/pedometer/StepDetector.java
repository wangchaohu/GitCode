package com.example.zry.pedometer;
		...
		if (gravityOld == 0) {
			...
			if (DetectorPeak(values, gravityOld)) {
				...
				if (timeOfNow - timeOfLastPeak >= 200 && (peakOfWave - valleyOfWave >= ThreadValue)
						&& timeOfNow - timeOfLastPeak <= 1000000) {
					timeOfThisPeak = timeOfNow;
					// 更新界面的处理，不涉及到算法
//					preStep();
					TEMP_STEP++;
					Log.d(TAG, "计算步数-------------" + TEMP_STEP);
...
////			TEMP_STEP++;
////			Log.d(TAG, "计步中 TEMP_STEP:" + TEMP_STEP);
